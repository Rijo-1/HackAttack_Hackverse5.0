...
import { Timer, TrendingUp, Trophy, AlertTriangle, Activity, X } from 'lucide-react';
...
const ConfirmModal: React.FC<ConfirmModalProps> = ({ isOpen, onClose, onConfirm, message }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div className="relative bg-black/90 rounded-xl border border-purple-500/20 p-6 max-w-md w-full">
        ...
        <div className="flex justify-end gap-3">
          <button
            onClick={onClose}
            className="px-4 py-2 rounded-lg bg-purple-500/10 hover:bg-purple-500/20 text-purple-300 transition-colors"
          >
            Cancel
          </button>
          <button
...
export const TradingGame: React.FC<TradingGameProps> = ({ team, onClose }) => {
  ...
  const handleTrade = (playerName: string, tradeData: any) => {
    if (socket) {
      ...
      try {
        const carbonFootprint = calculateCarbonFootprint(tradeData.amount, tradeData.leverage);
        const points = calculateFinalPoints(player.pnl, carbonFootprint);
  ...
  return (
    <div className="fixed inset-0 bg-black/90 backdrop-blur-sm z-50 overflow-y-auto">
      ...
      <ConfirmModal
        ...
        message="Are you sure you want to end the game early? This action cannot be undone."
      />
    </div>
  );
};
