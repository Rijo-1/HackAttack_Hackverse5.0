...
import { calculateCarbonFootprint, calculateFinalPoints } from '../../utils/trading/carbonFootprint';
...
export const GameLeaderboard: React.FC<GameLeaderboardProps> = ({
  ...
  const sortedPlayers = [...players].sort((a, b) => {
    const aPoints = calculateFinalPoints(
      a.pnl,
      a.trades.reduce((sum, trade) =>
        sum + calculateCarbonFootprint(trade.amount, trade.leverage), 0)
    );
    const bPoints = calculateFinalPoints(
      b.pnl,
      b.trades.reduce((sum, trade) =>
        sum + calculateCarbonFootprint(trade.amount, trade.leverage), 0)
    );
        ...
        <div className="space-y-3">
          {sortedPlayers.map((player, index) => (
            <button
              ...
              disabled={gameEnded}
              className={`w-full p-3 rounded-lg transition-all ${player.name === currentPlayer
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-2 text-sm">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-purple-400">{calculateFinalPoints(
                    player.pnl,
                    player.trades.reduce((sum, trade) =>
                      sum + calculateCarbonFootprint(trade.amount, trade.leverage), 0)
                  )}</span>
                </div>
              </div>
